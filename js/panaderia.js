...
function applyFilters() {
  const all = window._panaderiaAll || [];
  ...
  if (!list.length) {
    grid.innerHTML = `
      <div class="no-results">
        ...
        <p>No encontramos productos de panadería con esos filtros.</p>
...
/* ─── Contadores de subcategorías (PANADERÍA) ─── */
function updateSubcatCounts(prods) {
  const counts = {};
  prods.forEach(p => { counts[p.subcat] = (counts[p.subcat] || 0) + 1; });
  
  // Subcategorías de PANADERÍA
  const subcats = [
    'panes', 'facturas', 'tortillas','galletitas', 'budines', 'tostadas', 'prepizzas','sin-tacc'];
...
function addToCart(docId, e) {
  const prods = window._panaderiaAll || [];
...
function changeQty(docId, d) {
  ...
  const p = (window._panaderiaAll || []).find(x => x.docId === docId);
  const stock = p?.stock ?? null;
